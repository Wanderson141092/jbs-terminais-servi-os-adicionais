...
interface ModelosExcelDialogProps {
  open: boolean;
  onClose: () => void;
}

const ModelosExcelDialog = ({ open, onClose }: ModelosExcelDialogProps) => {
  ...
  return (
    <>
      <Dialog open={open && !showUploadDialog && !mappingModeloId} onOpenChange={() => onClose()}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <FileSpreadsheet className="h-5 w-5" />
              Modelos Excel
              <Badge variant="secondary" className="text-xs ml-2">{modelos.length} modelo(s)</Badge>
            </DialogTitle>
          </DialogHeader>
...
export default ModelosExcelDialog;
