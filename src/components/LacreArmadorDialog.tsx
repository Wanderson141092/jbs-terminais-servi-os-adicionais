...
import { Lock, Check, X, Download, Eye, RefreshCw, Clock } from "lucide-react";
...
const LacreArmadorDialog = ({ solicitacao, userId, onClose }: LacreArmadorDialogProps) => {
  const [documents, setDocuments] = useState<LacreDoc[]>([]);
  ...
  const fetchDocuments = async () => {
    const { data } = await supabase
      ...
      .eq("document_type", "lacre_armador")
      .order("created_at", { ascending: false });
    
    const docsWithSignedUrls: LacreDoc[] = [];
    if (data) {
  ...
  const calculateGeneralStatus = (docs: LacreDoc[]): "recebido" | "recusado" | "aguardando" => {
  ...
  const handleAceitar = async (docId: string) => {
    ...
    if (error) {
      toast.error("Erro ao aceitar documento de lacre");
      setLoading(false);
      return;
    }
    ...
    await logAudit("lacre_armador_aceito", `Lacre armador aceito. Arquivo: ${documents.find(d => d.id === docId)?.file_name || docId}. Status geral: ${generalStatus}`);
    await createNotification(`Lacre armador aceito para ${solicitacao.protocolo}`, "lacre_armador");
    toast.success("Lacre armador aceito! Status: Recebido");
    setLoading(false);
    fetchDocuments();
  };
  ...
  const handleRecusar = async (docId: string) => {
    ...
    if (error) {
      toast.error("Erro ao recusar documento de lacre");
      setLoading(false);
      return;
    }

    await logAudit("lacre_armador_recusado", `Lacre armador recusado. Arquivo: ${documents.find(d => d.id === docId)?.file_name || docId}. Motivo: ${motivoRecusa}`);
    await createNotification(`Lacre armador recusado para ${solicitacao.protocolo}`, "lacre_armador");
    toast.success("Lacre armador recusado!");
  ...
  return (
    <>
      <Dialog open onOpenChange={() => onClose()}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Análise de documentos de lacre armador
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm bg-muted/50 rounded-lg p-4">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Possui Lacre</p>
                <p className="font-medium">{solicitacao.lacre_armador_possui === true ? "Sim" : solicitacao.lacre_armador_possui === false ? "Não" : "—"}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Aceite Custo</p>
                <p className="font-medium">{solicitacao.lacre_armador_aceite_custo === true ? "Aceito" : "—"}</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <p className="text-sm font-semibold flex items-center gap-2">
                ...
                Documentos de Lacre Armador
              </p>

              {documents.length === 0 ? (
                <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4 text-center">
                  <Clock className="h-5 w-5 text-yellow-600 mx-auto mb-2" />
                  <p className="text-sm font-medium text-yellow-700">Aguardando envio do documento</p>
                  <p className="text-xs text-yellow-600 mt-1">Nenhum documento de lacre armador enviado pelo cliente.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {documents.map((doc) => (
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        ...
                        <div className="flex gap-2">
                          {(!doc.status || doc.status === "pendente") && (
                            <>
                              <Button variant="outline" size="sm" className="text-green-600 border-green-300 hover:bg-green-50" onClick={() => handleAceitar(doc.id)} disabled={loading}>
                                <Check className="h-4 w-4 mr-1" /> Aceitar
                              </Button>
                              <Button variant="outline" size="sm" className="text-red-600 border-red-300 hover:bg-red-50" onClick={() => setShowRecusaDialog(doc.id)} disabled={loading}>
                                <X className="h-4 w-4 mr-1" /> Recusar
                              </Button>
                            </>
                          )}
                          {doc.status === "aceito" && (
                            <Button variant="outline" size="sm" className="text-orange-600 border-orange-300 hover:bg-orange-50" onClick={() => setShowRecusaDialog(doc.id)} disabled={loading}>
                              <RefreshCw className="h-4 w-4 mr-1" /> Alterar para Recusado
                            </Button>
                          )}
                          {doc.status === "recusado" && !documents.some(d => d.id !== doc.id && new Date(d.created_at) > new Date(doc.created_at)) && (
                            <Button variant="outline" size="sm" className="text-green-600 border-green-300 hover:bg-green-50" onClick={() => handleAceitar(doc.id)} disabled={loading}>
                              <RefreshCw className="h-4 w-4 mr-1" /> Alterar para Recebido
                            </Button>
                          )}
                          {!showEmbeddedPreview && (
                      ...
                      {showEmbeddedPreview && (
                        <div className="bg-muted/30 rounded overflow-hidden">
                          {doc.file_url.toLowerCase().endsWith(".pdf") ? (
                            <iframe src={doc.file_url} className="w-full h-[300px]" title={doc.file_name} />
                          ) : (
                            <img src={doc.file_url} alt={doc.file_name} className="max-w-full max-h-[300px] mx-auto" />
                          )}
                        </div>
                      )}
      ...
      <Dialog open={!!showRecusaDialog} onOpenChange={() => setShowRecusaDialog(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Recusar Lacre Armador</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowRecusaDialog(null)}>Cancelar</Button>
            <Button variant="destructive" onClick={() => showRecusaDialog && handleRecusar(showRecusaDialog)} disabled={!motivoRecusa.trim() || loading}>
              Confirmar Recusa
            </Button>
          </DialogFooter>
        </DialogContent>
